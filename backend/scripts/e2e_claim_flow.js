...
const createClaim = async (token, memberId) => {
...
const advanceClaim = async (token, claimId, role) => {
...
(async () => {
  try {
    ...
    console.log('Logged in roles:', leader.user.role, orgAdmin.user.role, finance.user.role);
    ...
    console.log('Advanced to', step1.status);

    const step2 = await advanceClaim(orgAdmin.accessToken, claim.id, 'org_admin');
    console.log('Advanced to', step2.status);

    const step3 = await advanceClaim(orgAdmin.accessToken, claim.id, 'org_admin');
    console.log('Advanced to', step3.status);

    const step4 = await advanceClaim(finance.accessToken, claim.id, 'finance');
    console.log('Advanced to', step4.status);

    console.log('Final status:', step4.status);
  } catch (error) {
    console.error('E2E workflow failed:', error.message || error);
    process.exit(1);
  }
})();
