...
class ClaimsService {
    async createClaim(data, userId) {
        ...
        try {
         ...
         VALUES ($1, NULL, 'reported'::claim_status, $2, $3, NOW())`, [claimId, userId, 'Claim reported']);
    ...
    async advanceClaim(claimId, userId) {
        const client = await (0, database_1.getClient)();
        try {
            await client.query('BEGIN');
         ...
         WHERE id = $3 RETURNING *`, [nextStatus, userId, claimId]);
            await client.query(`INSERT INTO claim_workflow (claim_id, from_status, to_status, changed_by, comments, created_at)
         VALUES ($1, $2::claim_status, $3::claim_status, $4, $5, NOW())`, [claimId, currentStatus, nextStatus, userId, `Advanced from ${currentStatus} to ${nextStatus}`]);
            if (nextStatus === 'paid') {
                const claim = currentRes.rows[0];
                const transactionNumber = `PAY-${Date.now()}-${Math.floor(Math.random() * 10000)}`;
                await client.query(`INSERT INTO transactions (transaction_number, type, category, amount, member_id, claim_id, description, organization_id, created_at)
           VALUES ($1, 'debit', 'claim_payout', $2, $3, $4, 'Death benefit payout', $5, NOW())`, [transactionNumber, claim.amount, claim.member_id, claimId, claim.organization_id]);
            }
            await client.query('COMMIT');
